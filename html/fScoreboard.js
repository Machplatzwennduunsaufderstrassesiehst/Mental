...
function listSuggestions(suggestions) {
    ...
    for (var i = 0; i < suggestions.length; i++) {
        ...
        html += "<div style='width:"+scoreboardWidth+"px;"+(suggestion.highlight ? "background-color: #aaa;" : "")+"' class='selectListItem suggestion' onclick='vote("+suggestion.suggestionID+");'>";
...
var updateScoreboardObserver = new Observer("scoreboard", function(msg) {
    ...
    for (var i = 0; i < msg.scoreboard.length; i++) {
        ...
        html += "<tr style='"+(e.highlight ? "background-color: #e5ebff;" : "")+"'><td>"+(i+1)+"</td> \
                <td><p style='margin:0px 0px -4px 0px;color:"+e.color+";'>"+name+"</p><span style='font-weight:bold;font-size:0.65em;float:left;'>&#060;"+e.title+"&#062;</span>\
