/* global DEBUG_ALERT, DEBUG_ONPAGE, DEBUG */
...
var ajax = (function () {
    ...
    /* timeout the module will wait for a response until it assumes that a file does not exist */
    var response_timeout = 3000;

    // creates a platform-dependent xhttp object
    ...
    function queryContent(url, callback) {
    ...
    return {
        ...
        getFile: queryContent,
        loadContentTo: function (url, elementId) {
            queryContent(url, function (content) {
                byID(elementId).innerHTML = content;
            });
        }
    };
})();
...
function countdown(countdownId, value, recall, onCountdown, template) {
    ...
    if (value < 0) {
        ...
        if (recall) setTimeout(function () {
            countdown(countdownId, Number(value) - 1, true, onCountdown, template);
        }, 1000);
...
setInterval(function () {
    countdown("exerciseCountdown", exerciseCountdownValue, false);
    exerciseCountdownValue--;
}, 1000);
